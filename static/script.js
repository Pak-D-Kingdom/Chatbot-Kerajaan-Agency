document.addEventListener('DOMContentLoaded', () => {
    ...
    startChatBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.session_id) {
                sessionStorage.setItem('kerajaan_session', data.session_id);
            }
    ...
    function showChatUI(customerName) {
        ...
        setTimeout(() => {
            if (chatBody.children.length === 0) {
                ...
                addMessage(`Halo kak ${greetingName}! 👑 Selamat datang di Kerajaan Agency.\n\nKami adalah ekosistem Creator-Powered Commerce (Connect • Create • Grow). Ada yang bisa kami bantu seputar profil agensi, peluang KOL/creator, atau cara bergabung?`, 'bot');
            }
        }, 500);
    }
        ...
        if (!chatContainer.classList.contains('hidden')) {
            ...
            try {
                const res = await fetch('/api/session', {
                    headers: {
                        'X-Session-ID': sessionStorage.getItem('kerajaan_session') || ''
                    }
                });
                const data = await res.json();
    ...
    async function sendMessage(text) {
        ...
        try {
            ...
            // Render bot message with CTA button if present
            const ctaBtn = data.show_cta && data.cta_url ? data.cta_url : null;
            addMessage(data.reply, 'bot', true, ctaBtn);
